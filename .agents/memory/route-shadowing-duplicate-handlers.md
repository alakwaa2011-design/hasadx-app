...
A follow-up full-repo audit found a second instance of the same pattern: `assignments.ts` (mounted early) still had its own older create/lookup/participants/notes handlers for `/solo-challenges*` shadowing the newer versions in `solo-challenges.ts` (mounted late) — removed the stale ones and left only the non-overlapping `/deadline` route in `assignments.ts`. When auditing, watch for false positives from routers mounted with an explicit path prefix (e.g. `router.use("/ai-chat", aiChatRouter)`): an identical-looking path string in two files is not a real collision if one of them is prefixed at mount time.
