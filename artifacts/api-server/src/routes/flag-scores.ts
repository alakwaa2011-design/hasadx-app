...
import { db, flagScoresTable, studentAccountsTable } from "@workspace/db";
...
router.get("/api/flag-scores", async (_req, res) => {
  try {
    const scores = await db
      .select()
      .from(flagScoresTable)
      .orderBy(desc(flagScoresTable.score))
      .limit(50);
    res.json(scores);
  } catch (err) {
...
router.post("/api/flag-scores", async (req, res) => {
  try {
