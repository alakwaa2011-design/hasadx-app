...
export default function FlagsMultiPlay() {
  const { lang } = useI18n();
  ...
  useEffect(() => {
    ...
    socket.on("connect", () => {
      if (isHost) {
        socket.emit("flag:rejoin-host", { pin }, (res: any) => {
          if (res?.error) { setError(res.error); return; }
          setPhase("countdown");
        });
      } else {
        socket.emit("flag:join-game", { pin, name: playerName }, (res: any) => {
    ...
    socket.on("flag:countdown", (data: { seconds: number }) => {
      setPhase("countdown");
    ...
    socket.on("flag:question", (data: any) => {
      setPhase("question");
    ...
    socket.on("flag:answer-update", (data: { answeredCount: number; totalPlayers: number }) => {
      setAnsweredCount(data.answeredCount);
      setTotalPlayers(data.totalPlayers);
    });

    socket.on("flag:question-ended", (data: any) => {
      if (timerRef.current) clearInterval(timerRef.current);
      setLeaderboard(data.leaderboard || []);
      setPhase("leaderboard");
    });

    socket.on("flag:game-finished", (data: any) => {
      if (timerRef.current) clearInterval(timerRef.current);
  ...
  const handleAnswer = (code: string) => {
    ...
    socketRef.current.emit("flag:submit-answer", { pin, answer: code }, (res: any) => {
      if (res.error) return;
      setFeedback(res);
  ...
  const myRank = leaderboard.find(l => l.name === myName)?.rank || 0;

  if (phase === "connecting") {
    return (
      <Layout>
        <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-sky-50 via-slate-50 to-indigo-100 dark:from-gray-900 dark:via-slate-900 dark:to-gray-950" dir={dir}>
          {error ? (
            <div className="text-center px-6">
              <div className="text-5xl mb-4">😕</div>
              <p className="text-red-500 dark:text-red-400 font-bold text-lg mb-4">{error}</p>
              <button onClick={() => setLocation("/game/flags")} className="px-6 py-3 rounded-xl bg-sky-600 text-white font-bold">{lang === "ar" ? "العودة" : "Go Back"}</button>
            </div>
          ) : (
            <div className="text-center">
              <div className="w-12 h-12 rounded-full border-4 border-sky-500/30 border-t-sky-500 animate-spin mx-auto mb-4" />
  ...
  if (phase === "waiting") {
    return (
      <Layout>
        <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-sky-50 via-slate-50 to-indigo-100 dark:from-gray-900 dark:via-slate-900 dark:to-gray-950" dir={dir}>
          <div className="text-center px-6">
            <motion.div animate={{ y: [0, -10, 0] }} transition={{ repeat: Infinity, duration: 2 }} className="text-6xl mb-5">🏁</motion.div>
            ...
            <div className="bg-white dark:bg-white/10 border border-gray-200 dark:border-white/20 rounded-2xl px-6 py-4 mb-4 shadow-sm dark:shadow-none">
              <p className="text-sky-500 dark:text-sky-400 font-black text-xl">{playerName}</p>
            </div>
  ...
  if (phase === "countdown") {
    return (
      <Layout>
        <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-sky-50 via-slate-50 to-indigo-100 dark:from-gray-900 dark:via-slate-900 dark:to-gray-950 relative overflow-hidden" dir={dir}>
          <motion.div
            key="countdown-bg"
            animate={{ scale: [1, 1.5, 1], opacity: [0.1, 0.3, 0.1] }}
            transition={{ repeat: Infinity, duration: 1 }}
            className="absolute inset-0 bg-sky-500 rounded-full scale-0 pointer-events-none"
          />
          <AnimatePresence mode="wait">
            ...
            >
              <div className={`text-9xl font-black mb-4 ${countdownNum <= 0 ? "text-sky-500 dark:text-sky-400" : "text-gray-900 dark:text-white"}`}>
  ...
  if (phase === "finished") {
    ...
    const myResult = players.find((p: any) => p.name === myName);
    const myFinalRank = myResult?.rank || 0;

    return (
      <Layout>
        <div className="min-h-screen bg-gradient-to-br from-sky-50 via-slate-50 to-indigo-100 dark:from-gray-900 dark:via-slate-900 dark:to-gray-950 py-8 px-4" dir={dir}>
          <div className="max-w-lg mx-auto">
            ...
            {players.length > 0 && (
              <div className="mb-5">
                ...
                <div className="space-y-2">
                  {players.slice(1, 10).map((p: any, i: number) => {
                    const rank = i + 2;
                    ...
                    return (
                      <motion.div
                        ...
                        className={`flex items-center gap-3 p-3 rounded-xl border ${isMe ? "bg-sky-500/20 border-sky-500/40" : rank === 2 ? "bg-white dark:bg-white/10 border-gray-200 dark:border-white/15" : rank === 3 ? "bg-orange-500/10 border-orange-500/20" : "bg-gray-50 dark:bg-white/5 border-gray-100 dark:border-white/5"}`}
                      >
                        ...
                        <div className="flex-1">
                          <p className={`font-black text-sm ${isMe ? "text-sky-600 dark:text-sky-300" : "text-gray-900 dark:text-white"}`}>{p.name}{isMe && " 👈"}</p>
                          <p className="text-xs text-gray-400 dark:text-white/40">{p.correctCount}/{p.totalQuestions} {lang === "ar" ? "صحيح" : "correct"}</p>
                        </div>
                        <div className="text-end">
                          <p className={`font-black text-base ${isMe ? "text-sky-600 dark:text-sky-400" : "text-gray-800 dark:text-white/80"}`}>{p.score}</p>
            ...
            <div className="flex flex-col gap-3">
              <button onClick={() => setLocation("/game/flags")} className="w-full py-3.5 rounded-2xl bg-gradient-to-r from-sky-500 to-indigo-600 text-white font-black text-base shadow-lg flex items-center justify-center gap-2">
        ...
        <div className="min-h-screen bg-gradient-to-br from-sky-50 via-slate-50 to-indigo-100 dark:from-gray-900 dark:via-slate-900 dark:to-gray-950 py-8 px-4" dir={dir}>
          <div className="max-w-lg mx-auto">
            <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-5">
              ...
              {myLbEntry && (
                <div className="mt-2 inline-flex items-center gap-2 bg-sky-500/20 border border-sky-500/30 px-3 py-1.5 rounded-full">
                  <span className="text-sky-600 dark:text-sky-300 font-bold text-sm">{lang === "ar" ? `أنت: #${myLbEntry.rank}` : `You: #${myLbEntry.rank}`}</span>
              ...
              {leaderboard.slice(0, 8).map((entry, i) => {
                ...
                return (
                  <motion.div
                    ...
                    className={`flex items-center gap-3 p-3 rounded-xl border ${isMe ? "bg-sky-500/20 border-sky-500/40 shadow-lg shadow-sky-500/10" : i === 0 ? "bg-yellow-500/10 border-yellow-500/20" : "bg-gray-50 dark:bg-white/5 border-gray-100 dark:border-white/5"}`}
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <span className={`font-bold text-sm truncate block ${isMe ? "text-sky-600 dark:text-sky-300" : "text-gray-900 dark:text-white"}`}>{entry.name}{isMe && " 👈"}</span>
                      {entry.streak >= 3 && <span className="text-orange-400 text-xs">🔥 {entry.streak}</span>}
                    </div>
                    <span className={`font-black text-base ${isMe ? "text-sky-600 dark:text-sky-400" : "text-gray-800 dark:text-white/80"}`}>{entry.score}</span>
                  </motion.div>
                );
              })}
            </div>
  ...
  return (
    <Layout>
      <div className="min-h-screen bg-gradient-to-br from-sky-50 via-slate-50 to-indigo-100 dark:from-gray-900 dark:via-slate-900 dark:to-gray-950 py-4 px-4 relative overflow-hidden" dir={dir}>
        <AnimatePresence>
          {feedbackType && (
            <motion.div
            ...
            <AnimatePresence mode="wait">
              <motion.div key={questionIdx} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} transition={{ duration: 0.2 }}>
                <div className={`rounded-3xl shadow-xl overflow-hidden mb-4 border-2 transition-colors ${phase === "feedback" ? (feedback?.correct ? "border-green-500/60" : "border-red-500/60") : "border-gray-200 dark:border-white/10"}`}>
                  <div className="relative bg-white dark:bg-gradient-to-br dark:from-white/10 dark:to-white/5 p-6 flex items-center justify-center min-h-[210px]">
                    <motion.img
                      src={getFlagUrl(question.countryCode, 640)}
                      alt=""
                    ...
                    <AnimatePresence>
                      {phase === "feedback" && feedback?.correct && (
                        <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} exit={{ scale: 0 }} className="absolute inset-0 flex items-center justify-center pointer-events-none">
                          {["🎉", "⭐", "✨", "🌟", "💫"].map((e, i) => (
                            <motion.span key={i} initial={{ scale: 0, x: 0, y: 0 }} animate={{ scale: [0, 1.8, 0], x: (i - 2) * 55, y: [0, -80] }} transition={{ duration: 0.85, delay: i * 0.07 }} className="absolute text-2xl">
                              {e}
                            </motion.span>
                          ))}
                        </motion.div>
                  ...
                  <div className={`py-2.5 px-4 text-center text-sm font-bold text-gray-600 dark:text-white/70 ${phase === "feedback" && feedback?.correct ? "bg-green-500/30" : phase === "feedback" ? "bg-red-500/20" : "bg-gray-50 dark:bg-white/5"}`}>
                    ...
                    <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="flex items-center justify-center gap-2 mb-3">
                      {feedback.correct ? (
                        <>
                          ...
                          <span className="text-yellow-500 dark:text-yellow-400 font-black">+{feedback.score - myScore + feedback.score > 0 ? "100+" : "100"}</span>
                ...
                <div className="grid grid-cols-2 gap-3">
                  {question.options.map((opt: any, idx: number) => {
                    ...
                    const isCorrectOpt = feedback?.correctAnswer === opt.code;
                    ...
                    return (
                      ...
                      >
                        ...
                        <span className="text-sm font-black leading-tight">{lang === "ar" ? opt.nameAr : opt.nameEn}</span>
                        {isFb && isCorrectOpt && (
