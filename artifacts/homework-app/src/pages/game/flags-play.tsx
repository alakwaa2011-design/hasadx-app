...
import {
  Country, getCountriesByTier, getFlagUrl, shuffleArray, generateDistractors, LEVELS,
} from "@/data/countries";
...
export default function FlagsPlay() {
  const { lang } = useI18n();
  const dir = lang === "ar" ? "rtl" : "ltr";
  const [, setLocation] = useLocation();
  const search = useSearch();
  const params = new URLSearchParams(search);
  const { isArenaMode, myName, opponents, results: arenaResults, updateScore, finishArena } = useArena("flags");
  ...
  const levelInfo = LEVELS.find(l => l.tier === tier) || LEVELS[0];

  const [questions, setQuestions] = useState<{ country: Country; options: Country[] }[]>([]);
  ...
  useEffect(() => {
    const pool = getCountriesByTier(tier);
    const shuffled = shuffleArray(pool);
    const sel = shuffled.slice(0, count);
    const qs = sel.map(country => {
      const distractors = generateDistractors(country, pool);
  ...
  useEffect(() => {
    fetch(`${API_BASE}/api/flag-scores`)
      .then(r => r.ok ? r.json() : [])
  ...
  const refreshLeaderboard = async () => {
    try {
      const r = await fetch(`${API_BASE}/api/flag-scores`);
      if (r.ok) {
  ...
  if (questions.length === 0) {
    return (
      <Layout>
        <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-sky-50 via-blue-50 to-indigo-50 dark:from-gray-900 dark:via-slate-900 dark:to-gray-950">
          <div className="w-10 h-10 rounded-full border-4 border-sky-500/30 border-t-sky-500 animate-spin" />
        </div>
      </Layout>
    );
  }
            ...
            <div className="flex-1 max-w-lg mx-auto lg:mx-0 w-full">
              ...
              <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="bg-card border border-border/60 rounded-2xl p-5 shadow-md mb-4">
                <div className="grid grid-cols-2 gap-4">
                  <div className="text-center p-3 bg-sky-50 dark:bg-sky-950/20 rounded-xl">
                    <Target className="w-5 h-5 text-sky-600 mx-auto mb-1" />
                ...
                <div className="mt-4 pt-4 border-t border-border/40">
                  ...
                  <div className="w-full h-3 bg-gray-100 dark:bg-gray-800 rounded-full overflow-hidden">
                    <motion.div initial={{ width: 0 }} animate={{ width: `${accuracy}%` }} transition={{ delay: 0.5, duration: 0.8 }} className="h-full bg-gradient-to-r from-sky-500 to-indigo-500 rounded-full" />
                  </div>
                </div>
              </motion.div>
                ...
                <div className="space-y-2">
                  {results.map((r, i) => (
                    <div key={i} className={`flex items-center gap-3 p-2 rounded-xl ${r.correct ? "bg-green-50 dark:bg-green-950/10" : "bg-red-50 dark:bg-red-950/10"}`}>
                      ...
                      <span className="font-bold text-sm text-foreground flex-1">{lang === "ar" ? r.country.nameAr : r.country.nameEn}</span>
              ...
              <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }} className="flex flex-col gap-3">
                <ShareButtons
                  text={lang === "ar"
                    ? `🇸🇦🇺🇸🇯🇵${playerName.trim() ? ` ${playerName.trim()} -` : ""} حصلت على ${score} نقطة في لعبة أعلام الدول! (${accuracy}% صحيحة)\n🔥 سلسلة ${maxStreak} | ⏱ ${timeStr}\nجرّب تتغلب عليّ!`
                    : `🇸🇦🇺🇸🇯🇵${playerName.trim() ? ` ${playerName.trim()} -` : ""} I scored ${score} in World Flags Game! (${accuracy}% correct)\n🔥 Streak ${maxStreak} | ⏱ ${timeStr}\nTry to beat me!`}
                  url={window.location.origin + (import.meta.env.BASE_URL || "/") + "game/flags"}
                />
                <button onClick={handleReplay} className="w-full py-3.5 rounded-2xl bg-gradient-to-r from-sky-500 to-indigo-600 text-white font-black text-base shadow-lg flex items-center justify-center gap-2">
                  <RotateCcw className="w-5 h-5" />
                  {lang === "ar" ? "إعادة اللعب" : "Play Again"}
                </button>
                <button onClick={() => setLocation("/game/flags")} className="w-full py-3 rounded-2xl border border-border bg-background text-foreground font-bold text-sm flex items-center justify-center gap-2 hover:bg-muted transition-colors">
  ...
  const timerColor = isUrgent ? "text-red-400" : timer <= duration * 0.5 ? "text-amber-400" : "text-sky-400";

  return (
    <Layout>
      <div className="min-h-screen bg-gradient-to-br from-sky-50 via-blue-50 to-indigo-50 dark:from-gray-900 dark:via-slate-900 dark:to-gray-950 py-4 px-4 relative overflow-hidden" dir={dir}>
        ...
        <div className="max-w-lg mx-auto relative z-20">
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2">
              ...
              <button onClick={toggleMute} className="p-1.5 rounded-lg bg-black/10 dark:bg-white/10 text-gray-500 dark:text-white/70 hover:text-gray-900 dark:hover:text-white hover:bg-black/20 dark:hover:bg-white/20 transition-colors">
            ...
            <div className="flex items-center gap-3 text-sm">
              <AnimatePresence>
                {streak >= 3 && (
                  <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} exit={{ scale: 0 }} className="flex items-center gap-1 text-orange-500 dark:text-orange-400 font-black">
                    <Flame className="w-4 h-4" /> {streak}🔥
                  </motion.div>
                )}
              </AnimatePresence>
              <div className="font-black text-gray-900 dark:text-white">
                {score} <span className="text-xs text-gray-500 dark:text-white/50 font-medium">{lang === "ar" ? "نقطة" : "pts"}</span>
              </div>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 mb-2">
            <div className="flex-1 h-2 bg-black/10 dark:bg-white/10 rounded-full overflow-hidden">
              <motion.div className="h-full bg-gradient-to-r from-sky-400 to-indigo-500 rounded-full" animate={{ width: `${progress}%` }} transition={{ duration: 0.3 }} />
            </div>
            <span className="text-xs font-bold text-gray-500 dark:text-white/50">{currentIdx + 1}/{questions.length}</span>
          </div>

          <div className={`h-2 rounded-full overflow-hidden mb-4 ${isUrgent ? "bg-red-200 dark:bg-red-900/50" : "bg-black/10 dark:bg-white/10"}`}>
            <motion.div
              className={`h-full rounded-full transition-colors duration-300 ${isUrgent ? "bg-red-500" : timer <= duration * 0.5 ? "bg-amber-400" : "bg-sky-400"}`}
          ...
          <AnimatePresence mode="wait">
            ...
            >
              <div className={`rounded-3xl shadow-2xl overflow-hidden mb-4 border-2 transition-colors duration-200 ${isFeedback ? (selected === correctCode ? "border-green-500/60" : "border-red-500/60") : "border-black/10 dark:border-white/10"}`}>
                <div className="relative bg-gradient-to-br from-black/5 dark:from-white/10 to-black/3 dark:to-white/5 p-6 flex flex-col items-center justify-center min-h-[220px]">
                  <motion.img
                    key={q.country.code}
                    src={getFlagUrl(q.country.code, 640)}
                    alt={lang === "ar" ? "ما هذا العلم؟" : "What flag is this?"}
                    className="max-h-[170px] w-auto max-w-full rounded-xl shadow-2xl object-contain"
                    loading="eager"
                  ...
                  <div className={`absolute top-3 end-3 w-12 h-12 rounded-full flex items-center justify-center font-black text-base transition-all ${isUrgent ? "bg-red-500 text-white animate-pulse shadow-lg shadow-red-500/50" : "bg-black/20 dark:bg-white/20 text-gray-900 dark:text-white"}`}>
                    {Math.ceil(timer)}
                  </div>

                  <AnimatePresence>
                    {isFeedback && selected === correctCode && (
                      <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} exit={{ scale: 0 }} className="absolute inset-0 flex items-center justify-center pointer-events-none">
                        {["🎉", "⭐", "✨", "🌟", "💫", "🎊"].map((e, i) => (
                          <motion.span key={i} initial={{ scale: 0, x: 0, y: 0 }} animate={{ scale: [0, 1.8, 0], x: (i - 2.5) * 55, y: [0, -80 - Math.random() * 40] }} transition={{ duration: 0.85, delay: i * 0.06 }} className="absolute text-2xl">
                            {e}
                          </motion.span>
                        ))}
                      </motion.div>
                  ...
                  <AnimatePresence>
                    {isFeedback && selected !== correctCode && (
                      <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} className="absolute bottom-3 start-3">
                        <div className="bg-red-500 text-white text-xs font-black px-3 py-1.5 rounded-xl">
                          {lang === "ar" ? "❌ وقت انتهى أو خطأ" : "❌ Wrong!"}
                        </div>
                      </motion.div>
                    )}
                  </AnimatePresence>
                ...
                <div className={`py-2.5 px-4 text-center text-sm font-bold transition-colors ${isFeedback && selected === correctCode ? "bg-green-500/30 text-green-800 dark:text-white/70" : isFeedback ? "bg-red-500/20 text-gray-700 dark:text-white/70" : "bg-black/5 dark:bg-white/5 text-gray-600 dark:text-white/70"}`}>
                  {lang === "ar" ? "ما اسم هذه الدولة؟" : "Which country is this?"}
                </div>
              </div>
              ...
              <AnimatePresence>
                {isFeedback && pointsEarned && (
                  ...
                  >
                    ...
                    {streak >= 3 && <span className="text-orange-400 font-bold text-xs">🔥 سلسلة {streak}</span>}
                  </motion.div>
                )}
              </AnimatePresence>
              ...
              <div className="grid grid-cols-2 gap-3">
                {q.options.map((opt, idx) => {
                  ...
                  const isCorrectOpt = opt.code === correctCode;
