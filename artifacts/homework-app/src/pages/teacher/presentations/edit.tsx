...
import { SLIDE_THEMES, SLIDE_PATTERNS, getTheme, getPattern, isFreeTheme, isFreePattern, resolveSlideGradient, type CustomBackground } from "@/lib/slide-themes";

const API_BASE = import.meta.env.VITE_API_URL || "";

type SlideType =
  | "cover" | "content" | "bullets" | "quiz" | "activity" | "discussion"
  | "image" | "video" | "summary" | "objectives" | "warmup";
...
const TYPE_META: Record<SlideType, { ar: string; en: string; icon: typeof Type; color: string }> = {
  cover: { ar: "غلاف", en: "Cover", icon: LayoutIcon, color: "text-emerald-600" },
  objectives: { ar: "أهداف", en: "Objectives", icon: Target, color: "text-blue-600" },
  warmup: { ar: "تنشيط", en: "Warm-up", icon: Zap, color: "text-amber-600" },
  content: { ar: "محتوى", en: "Content", icon: Type, color: "text-slate-600" },
  bullets: { ar: "نقاط", en: "Bullets", icon: ListOrdered, color: "text-violet-600" },
  quiz: { ar: "اختبر", en: "Quiz", icon: HelpCircle, color: "text-rose-600" },
  activity: { ar: "نشاط", en: "Activity", icon: Gamepad2, color: "text-fuchsia-600" },
  discussion: { ar: "نقاش", en: "Discussion", icon: MessageSquare, color: "text-cyan-600" },
  image: { ar: "صورة", en: "Image", icon: ImageIcon, color: "text-orange-600" },
  video: { ar: "فيديو", en: "Video", icon: Video, color: "text-red-600" },
  summary: { ar: "ملخص", en: "Summary", icon: CheckCircle2, color: "text-green-600" },
};

function newSlide(type: SlideType): Slide {
  const id = `s_${Date.now().toString(36)}_${Math.random().toString(36).slice(2, 6)}`;
  const base: Slide = { id, type, emoji: "📄", title: "", speakerNotes: "" };
  if (type === "bullets" || type === "summary" || type === "objectives") base.bullets = [""];
  if (type === "content" || type === "warmup") base.body = "";
  if (type === "quiz") base.question = { text: "", optionA: "", optionB: "", optionC: "", optionD: "", correctAnswer: "A", explanation: "" };
  if (type === "activity") base.activity = { gameType: "wameed", instructions: "", questions: [] };
  if (type === "discussion") { base.discussionPrompt = ""; base.discussionPoints = [""]; }
...
export default function PresentationEditPage() {
  ...
  const [currentTeacher, setCurrentTeacher] = useState<{ isAdmin?: boolean; hasProDesign?: boolean } | null>(null);
  const isPro = Boolean(currentTeacher?.isAdmin || currentTeacher?.hasProDesign);
  ...
  useEffect(() => {
    ...
    fetch(`${API_BASE}/api/auth/me`, { credentials: "include" })
      .then(async (r) => { if (r.ok) setCurrentTeacher(await r.json()); })
      .catch(() => { /* non-fatal */ });
  ...
  const deleteSlide = (idx: number) => {
    if (!confirm(lang === "ar" ? "حذف هذه الشريحة؟" : "Delete this slide?")) return;
    setPres((prev) => {
      if (!prev) return prev;
      const slides = prev.slides.slice();
      slides.splice(idx, 1);
  ...
  const toggleShare = async () => {
    ...
    try {
      ...
      toast.success(next
        ? (lang === "ar" ? "أصبح العرض متاحاً للمشاركة" : "Sharing enabled")
        : (lang === "ar" ? "تم إيقاف المشاركة" : "Sharing disabled"));
    } catch {
  ...
  const copyPublicUrl = () => {
    navigator.clipboard
      ...
      .catch(() => {
        toast.error(lang === "ar" ? "تعذّر النسخ — انسخ يدويًا" : "Copy failed — copy manually");
      });
  };
  ...
  const exportPDF = () => {
    ...
    const slidesHtml = pres.slides.map((s) => {
      ...
      if (s.type === "cover") {
      ...
      } else if (s.type === "bullets" || s.type === "objectives" || s.type === "summary") {
        const items = (s.bullets || []).map((b) => `<li style="margin:12px 0;font-size:24px">${escape(b)}</li>`).join("");
        body = `
        ...
        const points = (s.discussionPoints || []).map((p) => `<li style="margin:10px 0;font-size:22px">${escape(p)}</li>`).join("");
        body = `
...
<body>
  ...
  <script>window.onload = () => { setTimeout(() => { window.print(); }, 300); };</script>
    ...
    if (!w) {
      toast.error(lang === "ar" ? "السماح بالنوافذ المنبثقة لتصدير PDF" : "Please allow pop-ups to export PDF");
      return;
    }
  ...
  const exportPPTX = async () => {
    ...
    try {
      ...
      /* Theme colors */
      const themeColors: Record<string, { bg1: string; bg2: string; text: string }> = {
      ...
      pres.slides.forEach((s) => {
        ...
        slide.addShape("rect", {
          x: 0, y: 6.7, w: 13.33, h: 0.8, fill: { color: tc.bg2 }, line: { type: "none" },
        });
        const emoji = s.emoji || "📄";
        const title = s.title || "";

        if (s.type === "cover") {
          slide.addText(emoji, { x: 0, y: 1.5, w: 13.33, h: 1.5, fontSize: 96, align: "center", color: tc.text });
          slide.addText(title || pres.title, { x: 0.5, y: 3.0, w: 12.33, h: 1.5, fontSize: 54, bold: true, align: "center", color: tc.text, fontFace: "Arial" });
          if (s.subtitle) slide.addText(s.subtitle, { x: 0.5, y: 4.5, w: 12.33, h: 0.7, fontSize: 24, align: "center", color: tc.text });
        } else if (s.type === "bullets" || s.type === "summary" || s.type === "objectives") {
          slide.addText(`${emoji} ${title}`, { x: 0.5, y: 0.4, w: 12.33, h: 0.9, fontSize: 36, bold: true, color: tc.text });
          slide.addText((s.bullets || []).map((b) => ({ text: b, options: { bullet: true, fontSize: 22, color: tc.text } })),
            { x: 0.8, y: 1.5, w: 11.5, h: 5.0, color: tc.text });
        } else if (s.type === "content" || s.type === "warmup") {
          slide.addText(`${emoji} ${title}`, { x: 0.5, y: 0.4, w: 12.33, h: 0.9, fontSize: 36, bold: true, color: tc.text });
          slide.addText(s.body || "", { x: 0.8, y: 1.7, w: 11.7, h: 4.7, fontSize: 22, color: tc.text });
        } else if (s.type === "quiz" && s.question) {
          const q = s.question;
          slide.addText(`❓ ${title}`, { x: 0.5, y: 0.3, w: 12.33, h: 0.7, fontSize: 26, color: "FBBF24" });
          slide.addText(q.text, { x: 0.5, y: 1.1, w: 12.33, h: 1.2, fontSize: 30, bold: true, color: tc.text });
          (["A","B","C","D"] as const).forEach((k, i) => {
            const isCorrect = q.correctAnswer === k;
            slide.addText(`${k}.  ${q[`option${k}` as "optionA"]}${isCorrect ? "  ✓" : ""}`, {
              x: 0.8 + (i % 2) * 6, y: 2.7 + Math.floor(i / 2) * 1.5, w: 5.5, h: 1.0,
              fontSize: 20, color: tc.text, bold: isCorrect,
              fill: isCorrect ? { color: "16A34A" } : { color: "FFFFFF", transparency: 85 },
              align: "left",
            });
          });
          if (q.explanation) {
            slide.addText(`💡 ${q.explanation}`, { x: 0.5, y: 5.8, w: 12.33, h: 0.7, fontSize: 16, color: tc.text, italic: true });
          }
        } else if (s.type === "activity" && s.activity) {
          slide.addText(`🎮 ${title}`, { x: 0.5, y: 0.5, w: 12.33, h: 0.9, fontSize: 40, bold: true, color: tc.text });
          slide.addText(s.activity.instructions || "", { x: 0.5, y: 1.7, w: 12.33, h: 1.5, fontSize: 22, color: tc.text });
          slide.addText(`${lang === "ar" ? "اللعبة" : "Game"}: ${s.activity.gameType}  ·  ${s.activity.questions.length} ${lang === "ar" ? "سؤال" : "questions"}`,
            { x: 0.5, y: 5.5, w: 12.33, h: 0.7, fontSize: 22, bold: true, color: tc.text });
        } else if (s.type === "discussion") {
          slide.addText(`💬 ${title || (lang === "ar" ? "نقاش" : "Discussion")}`, { x: 0.5, y: 0.4, w: 12.33, h: 0.9, fontSize: 28, color: "FBBF24" });
          slide.addText(s.discussionPrompt || "", { x: 0.5, y: 1.4, w: 12.33, h: 1.3, fontSize: 32, bold: true, color: tc.text });
          slide.addText((s.discussionPoints || []).map((p) => ({ text: p, options: { bullet: true, fontSize: 20, color: tc.text } })),
            { x: 0.8, y: 3.0, w: 11.5, h: 3.5, color: tc.text });
        } else if (s.type === "image" && s.imageUrl) {
          try { slide.addImage({ path: s.imageUrl, x: 1, y: 1, w: 11.33, h: 5.5, sizing: { type: "contain", w: 11.33, h: 5.5 } }); } catch { /* ignore */ }
          if (title) slide.addText(title, { x: 0.5, y: 6.0, w: 12.33, h: 0.6, fontSize: 18, color: tc.text, align: "center" });
        } else if (s.type === "video") {
          slide.addText(`🎬 ${title}`, { x: 0.5, y: 0.5, w: 12.33, h: 1, fontSize: 36, bold: true, color: tc.text });
          slide.addText(s.videoUrl || "", { x: 0.5, y: 3, w: 12.33, h: 1, fontSize: 18, color: tc.text, align: "center" });
        } else {
          slide.addText(`${emoji} ${title}`, { x: 0.5, y: 0.4, w: 12.33, h: 0.9, fontSize: 36, bold: true, color: tc.text });
        }
      ...
      toast.success(lang === "ar" ? "تم تحميل ملف PowerPoint!" : "PowerPoint downloaded!");
    } catch (e) {
      console.error(e);
      toast.error(lang === "ar" ? "تعذّر تصدير PowerPoint" : "PowerPoint export failed");
    } finally {
      setExporting(false);
    }
  };
  ...
  const aiFillSlide = async () => {
    ...
    if (!slide) return;
    if (!slide.title?.trim()) {
      toast.error(lang === "ar" ? "اكتب عنوان الشريحة أولاً" : "Add a slide title first");
      return;
    }
    setAiFilling(true);
    try {
      const r = await fetch(`${API_BASE}/api/presentations/${id}/ai-fill-slide`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        credentials: "include",
      ...
      toast.error(err instanceof Error && err.message !== "fail"
        ? err.message
        : (lang === "ar" ? "تعذّر ملء الشريحة" : "AI fill failed"));
    } finally {
      setAiFilling(false);
    }
  };
  ...
  const regenerateQuestions = async () => {
    ...
    try {
      const r = await fetch(`${API_BASE}/api/presentations/${id}/regenerate-questions`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        credentials: "include",
        body: JSON.stringify({
          slideId: slide.id,
          count: slide.type === "activity" ? Math.max(5, (slide.activity?.questions.length || 5)) : 1,
          topic: slide.title || pres.title,
        }),
      });
      if (!r.ok) throw new Error();
      const data = await r.json();
      const qs = data.questions as Array<{ text: string; optionA: string; optionB: string; optionC: string; optionD: string; correctAnswer: "A"|"B"|"C"|"D"; explanation?: string | null }>;
      if (slide.type === "quiz" && qs[0]) {
        updateSlide(activeIdx, { question: { ...qs[0], explanation: qs[0].explanation || "" } });
        ...
        updateSlide(activeIdx, {
          activity: {
            ...
            questions: qs.map((q) => ({ text: q.text, optionA: q.optionA, optionB: q.optionB, optionC: q.optionC, optionD: q.optionD, correctAnswer: q.correctAnswer })),
          },
        });
      }
      toast.success(lang === "ar" ? "✨ تم توليد الأسئلة!" : "Questions regenerated!");
    } catch {
  ...
  return (
    <Layout noHeader>
      ...
      <div className="sticky top-0 z-40 bg-card/95 backdrop-blur border-b border-border overflow-x-hidden">
        <div className="max-w-[1600px] mx-auto px-2 sm:px-5 py-2 flex items-center gap-1 sm:gap-2 min-w-0">
          <Link href="/teacher/presentations">
            <button title={lang === "ar" ? "رجوع" : "Back"} className="p-2 rounded-lg hover:bg-muted shrink-0">
              <ArrowLeft className={lang === "ar" ? "w-5 h-5 rotate-180" : "w-5 h-5"} />
            </button>
          </Link>

          <input
            value={pres.title}
            onChange={(e) => { setPres({ ...pres, title: e.target.value }); scheduleSave(); }}
            ...
            <div className="inline-flex items-stretch rounded-lg shadow overflow-hidden">
              <Link href={`/teacher/presentations/${pres.id}/present`}>
                <button
                  ...
                  title={lang === "ar" ? "العرض من البداية" : "Present from beginning"}
                >
                  <Play className="w-3.5 h-3.5 sm:w-4 sm:h-4" />
                  <span className="hidden sm:inline">{lang === "ar" ? "من البداية" : "From start"}</span>
                  <span className="sm:hidden">{lang === "ar" ? "بدء" : "Start"}</span>
                </button>
              </Link>
              <Link href={`/teacher/presentations/${pres.id}/present?from=${activeIdx}`}>
                <button
                  ...
                  title={lang === "ar" ? "العرض من الشريحة الحالية" : "Present from current slide"}
                >
                  <Play className="w-3.5 h-3.5 sm:w-4 sm:h-4" />
                  <span className="hidden sm:inline">{lang === "ar" ? "من هنا" : "From here"}</span>
                </button>
              </Link>
            </div>
          </div>
                ...
                {SLIDE_THEMES.map((t) => {
                  ...
                  return (
                    <button
                      ...
                      onClick={() => {
                        if (locked) {
                          toast(lang === "ar" ? "👑 ميزة احترافية — تواصل مع المسؤول" : "👑 Pro feature — ask your admin");
                          return;
                        }
                      ...
                      className={`relative h-10 w-16 sm:w-20 rounded-lg overflow-hidden ring-2 transition-all ${
                        pres.theme === t.key ? "ring-primary scale-105" : "ring-transparent hover:ring-border"
                      } ${locked ? "opacity-70" : ""}`}
                      title={locked ? (lang === "ar" ? "ميزة احترافية" : "Pro feature") : undefined}
                    >
                      <div className={`absolute inset-0 bg-gradient-to-br ${t.grad}`} />
                      <span className={`relative ${t.textOnLight ? "text-slate-800" : "text-white"} text-[10px] font-bold drop-shadow flex items-center justify-center h-full`}>
                ...
                {SLIDE_PATTERNS.map((p) => {
                  ...
                  return (
                    <button
                      ...
                      className={`relative h-10 w-16 sm:w-20 rounded-lg overflow-hidden ring-2 transition-all ${
                        pres.pattern === p.key ? "ring-primary scale-105" : "ring-transparent hover:ring-border"
                      } ${locked ? "opacity-70" : ""}`}
                      title={locked
                        ? (lang === "ar" ? "ميزة احترافية" : "Pro feature")
                        : isAi
                          ? (lang === "ar" ? "الذكاء الاصطناعي يختار خلفية لكل شريحة" : "AI picks a background per slide")
                          : undefined}
                    >
                      {isAi ? (
                        ...
                        <>
                          <div className={`absolute inset-0 bg-gradient-to-br ${themeMeta.grad}`} />
                          <div className="absolute inset-0" style={p.style} />
                        </>
                      )}
                      <span className={`relative ${(isAi ? false : themeMeta.textOnLight) ? "text-slate-800" : "text-white"} text-[10px] font-bold drop-shadow flex items-center justify-center h-full gap-0.5`}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[200px_1fr_320px] gap-4">
          ...
          <aside className="lg:max-h-[calc(100vh-8rem)] lg:overflow-auto space-y-2">
            <div className="text-xs font-bold text-muted-foreground px-1 mb-1">
              {lang === "ar" ? `الشرائح (${pres.slides.length})` : `Slides (${pres.slides.length})`}
            </div>
            {pres.slides.map((s, i) => {
              ...
              return (
                ...
                >
                  <div className="flex items-start gap-2">
                    <div className="text-[10px] font-bold text-muted-foreground w-5 mt-0.5">{i + 1}</div>
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="text-xs font-bold line-clamp-2">
                        {s.emoji} {s.title || (lang === "ar" ? "بدون عنوان" : "Untitled")}
                      </div>
                    </div>
                  </div>
                </button>
              ...
              <div className="space-y-3">
                ...
                <div className="flex items-center justify-between gap-2">
                  <div className="flex items-center gap-1.5 text-xs font-bold text-muted-foreground">
                    {(() => { const m = TYPE_META[active.type]; const I = m.icon; return <><I className={`w-4 h-4 ${m.color}`} /> {lang === "ar" ? m.ar : m.en}</>; })()}
                  </div>
                  <div className="flex items-center gap-1">
                    <button onClick={() => moveSlide(activeIdx, -1)} disabled={activeIdx === 0} className="p-1.5 rounded hover:bg-muted disabled:opacity-30" title={lang === "ar" ? "أعلى" : "Up"}>
                      <ChevronUp className="w-4 h-4" />
                    </button>
                    <button onClick={() => moveSlide(activeIdx, 1)} disabled={activeIdx === pres.slides.length - 1} className="p-1.5 rounded hover:bg-muted disabled:opacity-30" title={lang === "ar" ? "أسفل" : "Down"}>
                      <ChevronDown className="w-4 h-4" />
                    </button>
                    <button onClick={() => duplicateSlide(activeIdx)} className="p-1.5 rounded hover:bg-muted" title={lang === "ar" ? "نسخ" : "Duplicate"}>
                      <Copy className="w-4 h-4" />
                    </button>
                    <button onClick={() => deleteSlide(activeIdx)} disabled={pres.slides.length <= 1} className="p-1.5 rounded hover:bg-rose-50 dark:hover:bg-rose-950/40 text-rose-600 disabled:opacity-30" title={lang === "ar" ? "حذف" : "Delete"}>
            ...
            {active && (
              <>
                ...
                <button
                  ...
                  title={lang === "ar" ? "املأ محتوى الشريحة بناءً على العنوان" : "Fill slide content from the title"}
                >
                  {aiFilling ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Sparkles className="w-3.5 h-3.5" />}
                  {aiFilling
                    ? (lang === "ar" ? "جاري الملء…" : "Filling…")
                    : (lang === "ar" ? "املأ بالذكاء الاصطناعي" : "Fill with AI")}
                </button>

                <SlideInspector slide={active} updateSlide={(patch) => updateSlide(activeIdx, patch)} lang={lang} />
                ...
                {(active.type === "quiz" || active.type === "activity") && (
                  ...
                  >
                    ...
                    {regenerating
                      ...
                      : active.type === "quiz"
                        ? (lang === "ar" ? "ولّد سؤالاً جديداً بالذكاء الاصطناعي" : "Generate question with AI")
                        : (lang === "ar" ? "ولّد أسئلة جديدة بالذكاء الاصطناعي" : "Regenerate questions with AI")}
                  </button>
                )}
                ...
                <div>
                  ...
                  <textarea
                    ...
                    onChange={(e) => updateSlide(activeIdx, { speakerNotes: e.target.value })}
                    rows={3}
                    placeholder={lang === "ar" ? "ملاحظات تظهر فقط لك أثناء العرض" : "Visible only to you while presenting"}
      ...
      {/* Share dialog */}
      {showShareDialog && (
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4" onClick={() => setShowShareDialog(false)}>
          <div className="bg-card rounded-2xl shadow-2xl w-full max-w-md p-6 border border-border" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <h3 className="font-bold text-lg">{lang === "ar" ? "مشاركة العرض" : "Share Presentation"}</h3>
                <p className="text-xs text-muted-foreground">
                  {lang === "ar" ? "اجعل عرضك متاحاً للجميع برابط واحد" : "Make this deck publicly viewable with a single link"}
                </p>
              </div>
            </div>
            ...
            <div className="flex items-center justify-between p-4 rounded-xl bg-muted/50 border border-border mb-3">
              <div className="flex items-center gap-2">
                <Globe className={`w-5 h-5 ${pres.isShared ? "text-emerald-500" : "text-muted-foreground"}`} />
                <div>
                  ...
                  <div className="text-xs text-muted-foreground">
                    {pres.isShared
                      ? (lang === "ar" ? "أي شخص لديه الرابط يمكنه المشاهدة" : "Anyone with the link can view")
                      : (lang === "ar" ? "العرض خاص بك حالياً" : "Currently private")}
                  </div>
                </div>
              </div>
              <button
              ...
              >
                <span className={`inline-block h-4 w-4 transform rounded-full bg-white transition ${pres.isShared ? (lang === "ar" ? "-translate-x-6" : "translate-x-6") : "translate-x-1"}`} />
              </button>
            </div>
...
function SlidePreview({
  ...
  return (
    <div className={`relative rounded-2xl overflow-hidden shadow-2xl aspect-[16/9] bg-gradient-to-br ${themeGrad}`}>
      <div className={`absolute inset-0 ${textOnLight ? "bg-white/5" : "bg-black/10"}`} />
      {patternStyle && Object.keys(patternStyle).length > 0 && (
        <div className="absolute inset-0 pointer-events-none" style={patternStyle} />
      )}
      <div className={`relative w-full h-full ${textCls} p-6 sm:p-10 flex flex-col`}>
        {slide.type === "cover" && (
          <div className="m-auto text-center">
            <div className="text-7xl sm:text-8xl mb-4 drop-shadow-lg">{slide.emoji || "📚"}</div>
            <h1 className="text-3xl sm:text-5xl font-extrabold drop-shadow-lg">{slide.title || (lang === "ar" ? "عنوان الدرس" : "Lesson")}</h1>
            {slide.subtitle && <p className="text-white/90 mt-2 text-base sm:text-lg">{slide.subtitle}</p>}
          </div>
        )}

        {(slide.type === "objectives" || slide.type === "summary" || slide.type === "bullets") && (
          <>
            ...
            <ul className="space-y-3 text-base sm:text-xl">
              {(slide.bullets || []).map((b, i) => (
                <li key={i} className="flex items-start gap-3">
                  <span className="bg-white/20 backdrop-blur w-7 h-7 rounded-full flex items-center justify-center text-sm font-bold shrink-0">{i+1}</span>
                  <span>{b}</span>
                </li>
              ))}
            </ul>
        ...
        {(slide.type === "content" || slide.type === "warmup") && (
          <>
            ...
            <p className="text-base sm:text-xl leading-relaxed whitespace-pre-wrap flex-1">{slide.body}</p>
          </>
        )}
        ...
        {slide.type === "quiz" && slide.question && (
          <>
            ...
            <h3 className="text-xl sm:text-2xl font-bold mb-4">{slide.question.text}</h3>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-3">
              {(["A","B","C","D"] as const).map((k) => (
                <div key={k} className="bg-white/15 backdrop-blur border border-white/20 rounded-xl p-3 flex items-center gap-2">
                  <span className="bg-white/20 w-7 h-7 rounded-full flex items-center justify-center font-bold text-sm shrink-0">{k}</span>
                  <span className="text-sm sm:text-base">{slide.question![`option${k}` as "optionA"]}</span>
                </div>
              ))}
            </div>
          </>
        ...
        {slide.type === "activity" && slide.activity && (
          <>
            ...
            <p className="text-sm sm:text-base text-white/90 mb-4">{slide.activity.instructions}</p>
            <div className="mt-auto bg-black/30 backdrop-blur rounded-2xl p-4 inline-flex items-center gap-3 self-start">
              ...
              <div>
                ...
                <div className="font-bold capitalize">{slide.activity.gameType} · {slide.activity.questions.length} {lang === "ar" ? "سؤال" : "Qs"}</div>
              </div>
            </div>
          </>
        )}
        ...
        {slide.type === "discussion" && (
          <>
            ...
            <h2 className="text-2xl sm:text-3xl font-bold mb-4">{slide.discussionPrompt}</h2>
        ...
        {slide.type === "image" && (
          <div className="m-auto text-center">
            {slide.imageUrl
              ? <img src={slide.imageUrl} alt={slide.title || ""} className="max-w-full max-h-[60vh] mx-auto rounded-xl" />
              : <div className="text-white/60">{lang === "ar" ? "ضع رابط صورة" : "Set an image URL"}</div>}
          ...
          <div className="m-auto w-full max-w-3xl">
            {slide.videoUrl ? (
              <div className="aspect-video">
                <iframe src={toEmbed(slide.videoUrl)} className="w-full h-full rounded-xl" allowFullScreen />
              </div>
            ) : (
              <div className="text-white/60 text-center">{lang === "ar" ? "ضع رابط فيديو يوتيوب" : "Set a YouTube URL"}</div>
            )}
          </div>
        )}
      </div>
...
/* ─────────────────────────────────────────────
   ...
   ───────────────────────────────────────────── */
function SlideInspector({ slide, updateSlide, lang }: { slide: Slide; updateSlide: (p: Partial<Slide>) => void; lang: "ar" | "en" }) {
  const Label = ({ children }: { children: React.ReactNode }) => (
    <label className="block text-[10px] font-bold text-muted-foreground mb-1.5">{children}</label>
  );
  const cls = "w-full px-3 py-2 border border-border rounded-lg bg-card text-sm";

  if (slide.type === "cover") {
    return (
      <div>
        <Label>{lang === "ar" ? "العنوان الفرعي" : "Subtitle"}</Label>
        <input value={slide.subtitle || ""} onChange={(e) => updateSlide({ subtitle: e.target.value })} className={cls} />
      </div>
    );
  }
  ...
  if (slide.type === "content" || slide.type === "warmup") {
    return (
      <div>
        ...
        <textarea value={slide.body || ""} onChange={(e) => updateSlide({ body: e.target.value })} rows={6} className={`${cls} resize-none`} />
      </div>
    );
  }

  if (slide.type === "bullets" || slide.type === "summary" || slide.type === "objectives") {
    const bullets = slide.bullets || [];
    const update = (i: number, v: string) => { const arr = bullets.slice(); arr[i] = v; updateSlide({ bullets: arr }); };
    const remove = (i: number) => { const arr = bullets.filter((_, j) => j !== i); updateSlide({ bullets: arr }); };
        ...
        <div className="space-y-1.5">
          {bullets.map((b, i) => (
            <div key={i} className="flex gap-1">
              <input value={b} onChange={(e) => update(i, e.target.value)} className={cls} />
              <button onClick={() => remove(i)} className="p-2 text-rose-600 hover:bg-rose-50 dark:hover:bg-rose-950/40 rounded-lg"><Trash2 className="w-3.5 h-3.5" /></button>
            </div>
          ))}
          <button onClick={add} className="w-full inline-flex items-center justify-center gap-1 py-2 border border-dashed border-border rounded-lg text-xs hover:bg-muted">
            <Plus className="w-3 h-3" />{lang === "ar" ? "إضافة نقطة" : "Add point"}
          </button>
        </div>
      </div>
    );
  ...
  if (slide.type === "quiz") {
    const q = slide.question || { text: "", optionA: "", optionB: "", optionC: "", optionD: "", correctAnswer: "A" as const, explanation: "" };
    const setQ = (patch: Partial<typeof q>) => updateSlide({ question: { ...q, ...patch } });
    return (
      <div className="space-y-3">
        <div>
          <Label>{lang === "ar" ? "نص السؤال" : "Question"}</Label>
          <textarea value={q.text} onChange={(e) => setQ({ text: e.target.value })} rows={2} className={`${cls} resize-none`} />
        </div>
        {(["A","B","C","D"] as const).map((k) => (
          <div key={k}>
            <Label>{lang === "ar" ? `الخيار ${k}` : `Option ${k}`}</Label>
            <input value={q[`option${k}` as "optionA"]} onChange={(e) => setQ({ [`option${k}`]: e.target.value } as never)} className={cls} />
          </div>
        ))}
        <div>
          <Label>{lang === "ar" ? "الإجابة الصحيحة" : "Correct"}</Label>
          <select value={q.correctAnswer} onChange={(e) => setQ({ correctAnswer: e.target.value as "A"|"B"|"C"|"D" })} className={cls}>
            {(["A","B","C","D"] as const).map((k) => <option key={k} value={k}>{k}</option>)}
          </select>
        </div>
        <div>
          <Label>{lang === "ar" ? "شرح الإجابة" : "Explanation"}</Label>
          <textarea value={q.explanation || ""} onChange={(e) => setQ({ explanation: e.target.value })} rows={2} className={`${cls} resize-none`} />
        </div>
      </div>
    );
  }
  ...
  if (slide.type === "activity") {
    const a = slide.activity || { gameType: "wameed" as const, instructions: "", questions: [] };
    const setA = (patch: Partial<typeof a>) => updateSlide({ activity: { ...a, ...patch } });
    const updateQ = (i: number, patch: Partial<Question>) => { const arr = a.questions.slice(); arr[i] = { ...arr[i], ...patch }; setA({ questions: arr }); };
    const removeQ = (i: number) => setA({ questions: a.questions.filter((_, j) => j !== i) });
    const addQ = () => setA({ questions: [...a.questions, { text: "", optionA: "", optionB: "", optionC: "", optionD: "", correctAnswer: "A" }] });
    return (
      <div className="space-y-3">
        <div>
          <Label>{lang === "ar" ? "نوع اللعبة" : "Game"}</Label>
          <select value={a.gameType} onChange={(e) => setA({ gameType: e.target.value as typeof a.gameType })} className={cls}>
            <option value="wameed">⚡ {lang === "ar" ? "وميض (سؤال وجواب)" : "Wameed (Q&A)"}</option>
        ...
        <div>
          ...
          <textarea value={a.instructions || ""} onChange={(e) => setA({ instructions: e.target.value })} rows={2} className={`${cls} resize-none`} />
        </div>
        <div>
          <Label>{lang === "ar" ? `الأسئلة (${a.questions.length})` : `Questions (${a.questions.length})`}</Label>
          <div className="space-y-2">
            {a.questions.map((q, i) => (
              <details key={i} className="border border-border rounded-lg">
                <summary className="cursor-pointer px-2 py-1.5 text-xs font-bold flex items-center gap-1">
                  <span className="flex-1 truncate">{i + 1}. {q.text || (lang === "ar" ? "(فارغ)" : "(empty)")}</span>
                  <button onClick={(e) => { e.preventDefault(); removeQ(i); }} className="text-rose-600 p-1 hover:bg-rose-50 rounded"><Trash2 className="w-3 h-3" /></button>
                </summary>
                <div className="p-2 space-y-1.5">
                  <input value={q.text} onChange={(e) => updateQ(i, { text: e.target.value })} placeholder={lang === "ar" ? "السؤال" : "Question"} className={cls} />
                  {(["A","B","C","D"] as const).map((k) => (
                    <input key={k} value={q[`option${k}` as "optionA"]} onChange={(e) => updateQ(i, { [`option${k}`]: e.target.value } as never)} placeholder={k} className={cls} />
                  ))}
                  <select value={q.correctAnswer} onChange={(e) => updateQ(i, { correctAnswer: e.target.value as "A"|"B"|"C"|"D" })} className={cls}>
                    {(["A","B","C","D"] as const).map((k) => <option key={k} value={k}>{lang === "ar" ? `الإجابة: ${k}` : `Answer: ${k}`}</option>)}
                  </select>
                </div>
              </details>
            ))}
            <button onClick={addQ} className="w-full inline-flex items-center justify-center gap-1 py-2 border border-dashed border-border rounded-lg text-xs hover:bg-muted">
              <Plus className="w-3 h-3" />{lang === "ar" ? "إضافة سؤال" : "Add question"}
            </button>
          </div>
        </div>
      </div>
  ...
  if (slide.type === "discussion") {
    ...
    const update = (i: number, v: string) => { const arr = points.slice(); arr[i] = v; updateSlide({ discussionPoints: arr }); };
    const remove = (i: number) => updateSlide({ discussionPoints: points.filter((_, j) => j !== i) });
    const add = () => updateSlide({ discussionPoints: [...points, ""] });
    return (
      <div className="space-y-3">
        <div>
          <Label>{lang === "ar" ? "سؤال النقاش" : "Discussion prompt"}</Label>
          <textarea value={slide.discussionPrompt || ""} onChange={(e) => updateSlide({ discussionPrompt: e.target.value })} rows={2} className={`${cls} resize-none`} />
        </div>
        <div>
          ...
          <div className="space-y-1.5">
            {points.map((p, i) => (
              <div key={i} className="flex gap-1">
                <input value={p} onChange={(e) => update(i, e.target.value)} className={cls} />
                <button onClick={() => remove(i)} className="p-2 text-rose-600 hover:bg-rose-50 dark:hover:bg-rose-950/40 rounded-lg"><Trash2 className="w-3.5 h-3.5" /></button>
              </div>
            ))}
            <button onClick={add} className="w-full inline-flex items-center justify-center gap-1 py-2 border border-dashed border-border rounded-lg text-xs hover:bg-muted">
  ...
  if (slide.type === "image") {
    return (
      <div>
        ...
        <input value={slide.imageUrl || ""} onChange={(e) => updateSlide({ imageUrl: e.target.value })} className={cls} placeholder="https://…" />
      </div>
    );
  }
  ...
  if (slide.type === "video") {
    return (
      <div>
        ...
        <input value={slide.videoUrl || ""} onChange={(e) => updateSlide({ videoUrl: e.target.value })} className={cls} placeholder="https://youtube.com/…" />
      </div>
    );
  }
