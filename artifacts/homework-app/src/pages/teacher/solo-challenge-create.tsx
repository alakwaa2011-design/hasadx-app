...
import {
  ...
  Edit3, Save, X, AlertCircle, Settings, Layers, PenLine, Users, Volume2, VolumeX, Music, XCircle
} from "lucide-react";
...
function emptyQuestion(): Question {
  return { text: "", type: "mcq", optionA: "", optionB: "", optionC: "", optionD: "", correctAnswer: "A", fillAnswer: "", closeAnswers: "", difficulty: null, audioUrl: null };
}
...
function QuestionCard({
  ...
  if (!editing) {
    return (
      <div className="bg-card border border-border/60 rounded-2xl p-5 hover:border-primary/30 transition-colors shadow-sm group" dir="rtl">
        <div className="flex items-start justify-between gap-3">
          <div className="flex-1 min-w-0">
            <div className="flex items-start gap-2 mb-3 flex-wrap">
              <span className="w-6 h-6 rounded-md bg-primary/10 text-primary flex items-center justify-center text-xs font-black shrink-0 mt-0.5 border border-primary/20">{index + 1}</span>
              <span className="text-sm font-bold text-foreground leading-relaxed flex-1">{q.text}</span>
              {q.type === "fill_blank" && (
              ...
              <div className="flex gap-2">
                {([
                  ...
                  <div key={o.val} className={cn(
                    "flex-1 flex items-center justify-center gap-1.5 px-3 py-2.5 rounded-xl text-sm border",
          ...
          <div className="flex flex-col gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity">
            <button onClick={() => setEditing(true)} className="p-2 rounded-xl bg-muted/60 hover:bg-primary/10 text-muted-foreground hover:text-primary transition-colors border border-transparent hover:border-primary/20">
              <Edit3 className="w-4 h-4" />
            </button>
            <button onClick={onDelete} className="p-2 rounded-xl bg-muted/60 hover:bg-red-500/10 text-muted-foreground hover:text-red-500 transition-colors border border-transparent hover:border-red-500/20">
  ...
  return (
    <div className="bg-card border-2 border-primary/40 rounded-2xl p-5 shadow-sm relative overflow-hidden" dir="rtl">
      <div className="absolute top-0 start-0 w-full h-1 bg-gradient-to-r from-primary/60 to-primary/20" />
      <div className="flex items-center justify-between mb-4">
        <span className="text-xs font-black text-primary bg-primary/10 px-2.5 py-1 rounded-md border border-primary/20">سؤال {index + 1}</span>
        <div className="flex gap-2">
          {q.text.trim() && (
            <button onClick={() => setEditing(false)} className="px-3 py-1.5 rounded-xl bg-primary/10 text-primary font-bold text-xs hover:bg-primary hover:text-primary-foreground transition-colors flex items-center gap-1.5 border border-primary/20 hover:border-primary">
              <Save className="w-3.5 h-3.5" /> حفظ
            </button>
          )}
          <button onClick={onDelete} className="p-1.5 rounded-xl hover:bg-red-500/10 text-muted-foreground hover:text-red-600 transition-colors">
            <Trash2 className="w-4 h-4" />
          </button>
        </div>
      </div>
      <textarea
        ...
        rows={2}
        className="w-full text-sm font-bold rounded-xl px-4 py-3 bg-muted/50 border border-border/60 focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary/20 resize-none mb-4 text-foreground placeholder:text-muted-foreground transition-all shadow-sm"
      />
      ...
      {q.type === "fill_blank" ? (
        <div className="space-y-3 mb-4">
          <div>
            <label className="block text-[11px] font-black text-foreground mb-1.5">
            ...
            <input
              ...
              className="w-full text-sm font-bold rounded-xl px-4 py-3 bg-emerald-500/5 border-2 border-emerald-500/30 focus:outline-none focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500/20 text-foreground placeholder:text-muted-foreground transition-all"
              dir="rtl"
            />
          </div>
          <div>
            <label className="block text-[11px] font-black text-muted-foreground mb-1.5">
            ...
            <input
              ...
              className="w-full text-sm rounded-xl px-4 py-3 bg-muted/40 border border-border/60 focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary/20 text-foreground placeholder:text-muted-foreground transition-all"
              dir="rtl"
            />
          ...
          {opts.map((opt, oi) => (
            <div key={opt} className={cn(
              ...
              <button
                ...
                className={cn(
                  "w-8 h-8 rounded-lg flex items-center justify-center shrink-0 text-xs font-black transition-colors shadow-sm",
                  q.correctAnswer === opt
              ...
              <input
                ...
                className="flex-1 text-xs font-bold rounded-lg px-2 py-1.5 bg-transparent border-none focus:outline-none focus:ring-0 text-foreground placeholder:text-muted-foreground"
              />
            </div>
          ))}
        </div>
      ...
      <div className="flex items-center gap-2 mb-4 bg-muted/30 p-2 rounded-xl border border-border/40">
        <span className="text-[10px] font-bold text-muted-foreground ms-1 shrink-0">مستوى الصعوبة:</span>
