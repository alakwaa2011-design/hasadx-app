...
pattern3 = r'              \{/\* ══════════════════════════════════ STEP 3 — معاينة ونشر ══════════════════════════════════ \*/\}.*?              \{/\* Publish button \*/\}\n.*?                  \}\)\}\n                </motion\.div>\n              \)\}\n            </AnimatePresence>'
              ...
              {wizardStep === 3 && (
                <motion.div key="step3" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} transition={{ duration: 0.25 }} className="space-y-6">
                  ...
                  <button type="button" onClick={handlePublish} disabled={createMutation.isPending}
                    className="w-full py-4 rounded-2xl hover:opacity-95 text-white font-black text-base shadow-xl shadow-emerald-500/25 transition-all active:scale-[0.98] disabled:opacity-60 flex items-center justify-center gap-2 bg-emerald-500">
                  ...
                  {createMutation.isError && (
                    <p className="text-sm text-red-500 font-bold text-center">{(createMutation.error as Error)?.message}</p>
                  )}
                </motion.div>
              )}
            </AnimatePresence>'''
